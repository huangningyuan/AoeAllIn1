...
def deal_custom_bonus(data: DatFile, params: All_In_1_Params, civ_name):
    ...
    match civ_name:
        ...
        case 'Macedonians':
            ...
            set_require_techs(tech, params.switch_tech_id, params.castle_duplicate_tech_id)
            effect = get_new_effect(name)
            force_tech(effect, constants.ELITE_PHALANGITE_TECH_ID)
        ...
        case 'Puru':
            ...
            effect = get_new_effect(name)
            for unit in units:
                if unit and unit.class_ == 12 and unit.type_50 and unit.type_50.break_off_combat >= 32:
                    break_off_combat = unit.type_50.break_off_combat
                    if break_off_combat & 32 > 0:
                        plus_unit_attribute(effect, unit.id, -1, 63, -32)
                    if break_off_combat & 64 > 0:
